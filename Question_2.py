...
Support_data = "xls,spreadsheet;xlsx,spreadsheet;jpg,image"
worker = Support_data.strip().split(";")
worker = {
    i.split(",")[0]:i.split(",")[1]
     for i in worker
}
Primary_data = ["abc.jpg",  "xyz.xls", "text.csv","123" ]
result = {
    i:worker.get(i.split(".")[1],"unknown")
     if '.' in i 
     else "unknown" 
